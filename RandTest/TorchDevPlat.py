...
class BpNetWork(nn.Module):
    def __init__(self, taskName, layersDim, actFuncType = 'leakRelu'):
        ...
        self.elu = nn.ELU().to(self.device)
    ...
    def init_weights(self):
        for module in self.modules():
            if isinstance(module, nn.Linear):
                if self.actFuncType in ['elu', 'relu', 'leakRelu']:
                    nn.init.kaiming_normal_(module.weight, mode = 'fan_in', nonlinearity = 'leaky_relu')
                elif self.actFuncType in ['sigmoid', 'tanh']:
    ...
    def forward(self, inputData):
        ...
        for layer in self.layers:
            if (self.actFuncType == 'elu'):
                outputData = self.elu(layer(outputData)).to(self.device)
    ...
    def SetTrainDataInfo(self, inputData, labelData, isNeedShuffle = False):
        ...
        self.dataloader = DataLoader(data, batch_size = self.batchSize, drop_last = True, shuffle = isNeedShuffle)
...
class SimpleLstmNetWork(nn.Module):
    def __init__(self, taskName, isBatchFirst, isNeedHidden, isOutput, isBiDirectional, trainDataDim, labelDataDim, hiddenDim, layerNum, batchSize, dropRate, oriTrainNum = None, targetTrainNum = None):
        ...
        self.hidden2LabelDim = nn.Linear(hiddenDim, labelDataDim).to(self.device)
        if (isOutput) and (oriTrainNum is not None) and (targetTrainNum is not None):
            #print(f"SimpleLstmNetWork labelDataDim = {labelDataDim}, oriTrainNum = {oriTrainNum}, targetTrainNum = {targetTrainNum}")
    ...
    def TrainNeuralNetWork(self, epochNum, statPeriod):
        ...
        for epoch in range(epochNum):
            ...
            for trainData, labelData in self.dataloader:
                ...
                self.optimizer.zero_grad()
                output, (hn, cn) = self.forward(trainData)
                hn = self.hidden2LabelDim(hn)
                loss = self.criterion(hn, labelData)
    ...
    def GetModuleCalcRst(self, verifyData, isNeedGrad = False):
        #with torch.no_grad():
        with torch.set_grad_enabled(isNeedGrad):
            verifyData = verifyData.to(self.device)
            output, (hn, cn) = self.forward(verifyData)
            hn = self.hidden2LabelDim(hn)
            return hn, self.loss, self.varMinusRst.item()

def HandleSimpleLstmNetWorkProcess(taskName, isBatchFirst, isNeedHidden, isOutput, isBiDirectional, trainData, labelData, hiddenDim, layerNum, batchSize, dropRate, epochNum, learnRate, weightDecay, statPeriod, modulePath):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    model = SimpleLstmNetWork(taskName, isBatchFirst, isNeedHidden, isOutput, isBiDirectional, trainData.shape[2], labelData.shape[2], hiddenDim, layerNum, batchSize, dropRate, trainData.shape[1], labelData.shape[1]).to(device)
    if os.path.exists(modulePath):
        checkpoint = torch.load(modulePath)
...
class MemLstmNetWork(nn.Module):
    ...
    def HandleMemLstmInputStep(self, inputStep, memMask, layerIndex, hiddenStates, cellStates):
        ...
        cellState = torch.sigmoid(inputStep @ self.Wi[layerIndex] + prevHidden @ (memMask * self.Ui[layerIndex]))
        cellState = torch.clamp(cellState, min = -1, max = 1)
        forgetGate = torch.sigmoid(inputStep @ self.Wf[layerIndex] + prevHidden @ (memMask * self.Uf[layerIndex]))
        forgetGate = torch.clamp(forgetGate, min = -1, max = 1)
        outputGate = torch.sigmoid(inputStep @ self.Wo[layerIndex] + prevHidden @ (memMask * self.Uo[layerIndex]))
        outputGate = torch.clamp(outputGate, min = -1, max = 1)
        choiceGate = torch.tanh(inputStep @ self.Wg[layerIndex] + prevHidden @ (memMask * self.Ug[layerIndex]))
    ...
    def SetTrainDataInfo(self, inputData, labelData, isNeedShuffle = False):
        ...
        data = TensorDataset(inputData.to(self.device), labelData.to(self.device))
        # 加载数据并自动进行批处理
        self.dataloader = DataLoader(data, batch_size = self.batchSize, shuffle = isNeedShuffle)
...
class RecurrentRnnNetWork(nn.Module):
    def __init__(self, taskName, isBatchFirst, isNeedHidden, isOutput, isNeedMaskMem, isNonFunc, isBiDirectional, trainDataDim, labelDataDim, hiddenDim, layerNum, maxSeqLen, splitPartNum, crossLenRate, maxLevelNum, batchSize, dropRate, oriTrainNum = None, targetTrainNum = None):
        ...
        for i in range(int(self.maxLevelNum)):
            self.mergeModuleDict[str(i)] = SimpleLstmNetWork(self.taskName, self.isBatchFirst, self.isNeedHidden, False, self.isBiDirectional, self.trainDataDim, self.labelDataDim, self.mergeHiddenDim, self.layerNum, self.batchSize, self.resDropRate).to(self.device)
        '''

        '''
        if (isNeedMaskMem):
            #self.handleModule = MemRnnNetWork(self.taskName, self.isBatchFirst, self.isNeedMaskMem, self.trainDataDim, self.mergeHiddenDim, self.mergeHiddenDim, self.layerNum, self.batchSize, self.dropRate).to(self.device)
            self.handleModule = MemLstmNetWork(self.taskName, self.isBatchFirst, self.isBiDirectional, self.trainDataDim, self.mergeHiddenDim, self.mergeHiddenDim, self.layerNum, self.batchSize, self.dropRate).to(self.device)
        else:
            self.handleModule = SimpleLstmNetWork(self.taskName, self.isBatchFirst, self.isNeedHidden, False, self.isBiDirectional, self.trainDataDim, self.mergeHiddenDim, self.mergeHiddenDim, self.layerNum, self.batchSize, self.dropRate).to(self.device)
        '''
        ...
        self.handleNonFunc = BpNetWork(self.taskName, [self.batchSize, self.mergeHiddenDim, self.batchSize])
        self.outResLinear = BpNetWork(self.taskName, [self.trainDataDim, self.mergeHiddenDim, self.labelDataDim])
        self.train2HiddenDim = BpNetWork(self.taskName, [self.trainDataDim, self.mergeHiddenDim])
        self.hidden2LabelDim = BpNetWork(self.taskName, [self.mergeHiddenDim, self.labelDataDim])
        if (self.isOutput) and (oriTrainNum is not None) and (targetTrainNum is not None):
            #print(f"SimpleRnnNetWork labelDataDim = {labelDataDim}, oriTrainNum = {oriTrainNum}, targetTrainNum = {targetTrainNum}")
            self.ori2TargetNum = nn.Linear(oriTrainNum, targetTrainNum)  # 新增的线性层
    ...
    def SaveModuleWeight(self, modulePath):
        ...
        torch.save({'model_state_dict': self.state_dict(), 'varMinusRst': self.varMinusRst, 'loss': self.loss}, modulePath)
        #torch.save(self.state_dict(), modulePath)

    def LoadModuleWeight(self, modulePath):
        print(f"LoadModuleWeight modulePath = {modulePath}")
        checkpoint = torch.load(modulePath, weights_only = False)
        self.load_state_dict(checkpoint['model_state_dict'])
        self.varMinusRst = checkpoint['varMinusRst']
        self.loss = checkpoint['loss']
        return self

    def forward(self, input):
        #print(f"RecurrentRnnNetWork input = {input.shape}")
        input = torch.nan_to_num(input, nan = -1, posinf = 10, neginf = -10)
        input = torch.clamp(input, -10, 10)

        oriBatchSize = 0
        if (input.shape[0] < self.batchSize):
            oriBatchSize = input.shape[0]
            padInput = torch.zeros(self.batchSize - input.shape[0], input.shape[1], input.shape[2]).to(self.device)
            input = AddDataToTorch(input, padInput, 0)

        #'''
        if (self.isNonFunc) and (len(input.shape) == 3):
            input = input.transpose(0, 2)
            input = self.handleNonFunc(input)
            input = input.transpose(0, 2)
        #'''
        ...
        sequence = self.train2HiddenDim(input)
        '''

        #'''
        #normal mask
        input = input.to(self.device)
        input = GenerateNormalizeRst(input)
        sequence = self.train2HiddenDim(input)
        #'''

        sequence = self.TwoTowerForwardProcess(sequence)
        oriOutput = self.hidden2LabelDim(sequence)
        outputRes = self.outResLinear(input)
        output = oriOutput + outputRes
        ...
        if self.isOutput and hasattr(self, 'ori2TargetNum'):
            ...
            output = self.ori2TargetNum(output)
            output = output.transpose(1, 2)

        if (oriBatchSize > 0):
            output = output[0 : oriBatchSize, : , : ]

        #print(f"SnnNetWork forward self.isOutPut = {self.isOutPut}, self.labelDataDim = {self.labelDataDim}, inputData = {input.shape}, output = {output.shape}")
        return output

    def TwoTowerForwardProcess(self, sequence):
        #print(f"TwoTowerForwardProcess in sequence = {sequence.shape}")
        oriBatchSize, sequence, dataErrList = self.FirstTowerForardProcess(sequence)
        sequence = self.SecondTowerForwardProcess(sequence, oriBatchSize, dataErrList)
        #print(f"TwoTowerForwardProcess out sequence = {sequence.shape}")
        return sequence

    def FirstTowerForardProcess(self, sequence):
        #maxSeqLen 10; splitPartNum 20;lra sequence[32, 882, 128]
        # print(f"FirstTowerForardProcess in sequence = {sequence.shape}")
        dataErrList = []
        ...
        while (sequence.shape[1] > self.maxSeqLen):
            ...
            if (trainDataNum % self.splitPartNum != 0):
                ...
                dataErrList.append(dataNumErr)
            else:
                dataErrList.append(0)
        ...
        #print(f"FirstTowerForardProcess out sequence = {sequence.shape}")
        return oriBatchSize, sequence, dataErrList

    def SecondTowerForwardProcess(self, sequence, oriBatchSize, dataErrList):
        #print(f"SecondTowerForwardProcess in sequence = {sequence.shape}, oriBatchSize = {oriBatchSize}")
        isCalRrnn = False
        dataErrIndex = len(dataErrList) - 1
        ...
        while (oriBatchSize != batchSize) or (not isCalRrnn):
            isCalRrnn = True
            #print(f"SecondTowerForwardProcess sequence = {sequence.shape}")
            crossLen = int(self.crossLenRate * sequence.shape[1])
            if (crossLen > 0):
                #print(f"SecondTowerForwardProcess tempSeq = {sequence.shape}")
                rearCrossLen = crossLen
                frontCrossLen = sequence.shape[1] - crossLen - 1
                tempSeq = AddDataToTorch(sequence, sequence[ : , 0 : crossLen, : ], 1)
                tempSeq = AddDataToTorch(sequence[ : , sequence.shape[1] - crossLen : , : ], tempSeq, 1)
            else:
                tempSeq = sequence

            sequence, (_, _) = self.handleModule(tempSeq)  # sequence[12800, 3, 128]
            sequence = sequence[ : , crossLen : sequence.shape[1] - crossLen, : ]
            #_, (sequence, _) = self.handleModule(tempSeq)  # sequence[12800, 3, 128]
            ...
            if (batchSize > oriBatchSize):
                ...
                #print(f"sequence = {sequence.shape}, batchSize = {batchSize}, splitPartNum = {self.splitPartNum}, trainDataNum = {trainDataNum}")
                sequence = sequence.reshape(int(batchSize / self.splitPartNum), self.splitPartNum * trainDataNum, trainDataDim)
                if (dataErrIndex >= 0):
                    dataErr = dataErrList[dataErrIndex]
                    dataErrIndex = dataErrIndex - 1
                    sequence = sequence[ : , 0 : sequence.shape[1] - dataErr, : ]
        #print(f"SecondTowerForwardProcess out sequence = {sequence.shape}")
        return sequence
        ...
        for epoch in range(epochNum):
            ...
            for trainData, labelData in self.dataloader:
                ...
                if (len(trainData.shape) == 3) and ((len(labelData.shape) == 2) or (len(labelData.shape) == 3)):
                    #print(f"TrainNeuralNetWorkInEpoch trainData = {trainData.shape}")
                    output = self.forward(trainData)
                    #print(f"TrainNeuralNetWorkInEpoch output = {output.shape}, labelData = {labelData.shape}")
                    loss = self.criterion(output, labelData)
                    loss.backward(retain_graph=True)
                    # 生成计算图并保存为PDF文件
            ...
            if (epoch + 1) % statPeriod == 0:
                ...
                print(f"RecurrentRnnNetWork taskName = {self.taskName}, Epoch[{epoch + 1}/{epochNum}], trainLoss: {trainLoss}, current_lr = {current_lr}")
                #print(f"RecurrentRnnNetWork TrainNeuralNetWork checkRst = {checkRst}, varMinusRst = {varMinusRst}")
                #pass`
    ...
    def PrintDictGrad(self, dictName, paramDict):
        for key, parameters in paramDict.items():
            print(f"PrintDictGrad dictName = {dictName}, key = {key}, parameters.shape = {parameters.shape}")
            if parameters.grad is not None:
                #grad = torch.autograd.grad(loss, parameters, retain_graph = True, allow_unused = True)
                print(f"PrintDictGrad dictName = {dictName}, key = {key}, gradmean = {parameters.grad.mean()}, gradvar = {parameters.grad.var()}, max = {parameters.grad.max()}, min = {parameters.grad.min()}")
            else:
                print(f"PrintDictGrad dictName = {dictName}, key = {key} grad is None")
            print(f"PrintDictGrad dictName = {dictName}, key = {key}, mean = {parameters.mean()}, var = {parameters.var()}, max = {parameters.max()}, min = {parameters.min()}")

    def PrintSelfPara(self):
        params = dict(self.named_parameters())
        #print(params)
        for key, parameters in params.items():
            if parameters.grad is not None:
                print(f"PrintSelfPara key = {key}, mean = {parameters.mean():.10f}, var = {parameters.var():.10f}, max = {parameters.max():.10f}, min = {parameters.min():.10f}, gradmean = {parameters.grad.mean():.10f}, gradvar = {parameters.grad.var():.10f}")

    def GetModuleCalcRst(self, verifyData, isNeedGrad = False):
        #with torch.no_grad():
        with torch.set_grad_enabled(isNeedGrad):
            output = None
            self.SetTrainDataInfo(verifyData, torch.rand_like(verifyData), isNeedShuffle = False)
            for trainData, labelData in self.dataloader:
                #print(f"GetModuleCalcRst verifyData = {verifyData}")
                outputStep = self.forward(trainData)
                output = AddDataToTorch(output, outputStep, 0)
            #print(f"GetModuleCalcRst verifyData = {verifyData}, output = {output}")
            return output, self.loss, self.varMinusRst

def HandleRecurrentRnnNetWorkProcess(taskName, isBatchFirst, isNeedHidden, isOutput, isNeedMaskMem, isNonFunc, isBiDirectional, trainData, labelData, hiddenDim, layerNum, maxSeqLen, splitPartNum, crossLenRate, maxLevelNum, batchSize, dropRate, epochNum, learnRate, weightDecay, statPeriod, modulePath):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = RecurrentRnnNetWork(taskName, isBatchFirst, isNeedHidden, isOutput, isNeedMaskMem, isNonFunc, isBiDirectional, trainData.shape[2], labelData.shape[2], hiddenDim, layerNum, maxSeqLen, splitPartNum, crossLenRate, maxLevelNum, batchSize, dropRate, trainData.shape[1], labelData.shape[1]).to(device)
    #'''
    if os.path.exists(modulePath):
        model = model.LoadModuleWeight(modulePath)
        print(f"HandleRecurrentRnnNetWorkProcess taskName = {taskName}, modulePath = {modulePath}, model.loss = {model.loss}, model.varMinusRst = {model.varMinusRst}")
        #if (model.loss < 30) and (model.varMinusRst.item() < 100):
        return model
    else:
        print(f"HandleRecurrentRnnNetWorkProcess path not exist, path = {modulePath}")
    #'''
    model.SetCriterion(nn.MSELoss())
    #model.SetCriterion(nn.CrossEntropyLoss())
    model.SetOptimizer(torch.optim.Adam(model.parameters(), lr = learnRate, weight_decay = weightDecay))
...
#trainData = torch.randn(trainDataNum, timeStep, trainDataDim)
#labelData = torch.LongTensor(range(0, trainDataNum))
#model = HandleSadsNetWorkProcess(taskName, isBatchFirst, isNeedHidden, isOutput, trainData, labelData, lobeLabelDim, hiddenDim, cacheSize, maxSeqLen, splitPartNum, crossLenRate, batchSize, epochNum, dropRate, learnRate, weightDecay, statPeriod, varThreshold, pruneRate, modulePath)
# 使用模型进行预测
#verifyData = trainData
#model.GetModuleCalcRst(verifyData)
...
class SadsNetWork(nn.Module):
    def __init__(self, taskName, isBatchFirst, isNeedHidden, isOutput, isNonFunc, trainDataDim, labelDataDim, lobeLabelDim, hiddenDim, layerNum, maxSeqLen, splitPartNum, crossLenRate, maxLevelNum, cacheSize, maxLoopCount, batchSize, dropRate, oriTrainNum = None, targetTrainNum = None):
        ...
        self.isNonFunc = isNonFunc
        self.trainDataDim = trainDataDim
        self.lobeLabelDim = lobeLabelDim
        self.labelDataDim = labelDataDim
        self.hiddenDim = hiddenDim
        self.layerNum = layerNum
        self.maxSeqLen = maxSeqLen
        ...
        self.headNum = 20
        if (self.headNum != 0):
            self.multiheadAttention = MultiHeadAttention(self.hiddenDim, self.headNum, self.dropRate)
        '''

        #额叶
        self.parietalLobe = RecurrentRnnNetWork(self.taskName, self.isBatchFirst, self.isNeedHidden, False, False, self.isNonFunc, True, self.lobeLabelDim, self.lobeLabelDim, self.hiddenDim, self.layerNum, self.maxSeqLen, self.splitPartNum, self.crossLenRate, self.maxLevelNum, self.batchSize, self.dropRate).to(self.device)

        #颞叶
        self.temporalLobe = RecurrentRnnNetWork(self.taskName, self.isBatchFirst, self.isNeedHidden, False, True, self.isNonFunc, True, self.lobeLabelDim, self.lobeLabelDim, self.hiddenDim, self.layerNum, self.maxSeqLen, self.splitPartNum, self.crossLenRate, self.maxLevelNum, self.batchSize, self.dropRate).to(self.device)
    ...
    def forward(self, inputData):
        ...
        if (self.headNum != 0):
            ...
            #print(f"SadsNetWork sequence = {sequence.shape}, attentionOut = {attentionOut.shape}")
        ...
        # 额叶处理
        parietalOutput = self.parietalLobe(inputData)  # [batchSize, 1, hiddenDim]
        ...
        for index in range(self.loopCount):
            ...
            # 颞叶处理
            if (index == 0):
                temporalOutput = self.temporalLobe(inputData)
            else:
        ...
        # 额叶最后一次处理
        outputData = self.GetCacheLobeCalRst(cacheBuff, self.parietalLobe)
        ...
        if self.isOutput and hasattr(self, 'ori2TargetNum'):
            outputData = outputData.transpose(1, 2)
            outputData = self.ori2TargetNum(outputData)
            outputData = outputData.transpose(1, 2)

        #print(f"SadsNetWork forward self.isOutPut = {self.isOutPut}, self.labelDataDim = {self.labelDataDim}, inputData = {inputData.shape}, outputData = {outputData.shape}")
        return outputData

    def GetCacheLobeCalRst(self, cacheBuff, lobeModule):
        ...
        if len(inputData.shape) == 4:
            ...
            for i in range(0, inputData.shape[0]):
                ...
                sampleOutput = lobeModule(sampleInput)
    ...
    def GetModuleCalcRst(self, verifyData, isNeedGrad = False):
        ...
        with torch.set_grad_enabled(isNeedGrad):
            verifyData = verifyData.to(self.device)
            output = self.forward(verifyData)
            return output.cpu(), self.loss, self.varMinusRst

def HandleSadsNetWorkProcess(taskName, isBatchFirst, isNeedHidden, isOutput, isNonFunc, trainData, labelData, lobeLabelDim, hiddenDim, layerNum, maxSeqLen, splitPartNum, crossLenRate, maxLevelNum, cacheSize, maxLoopCount, batchSize, epochNum, dropRate, learnRate, weightDecay, statPeriod, modulePath):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"trainData = {trainData.shape}, labelData = {labelData.shape}, hiddenDim = {hiddenDim}, batchSize = {batchSize}")
    model = SadsNetWork(taskName, isBatchFirst, isNeedHidden, isOutput, isNonFunc, trainData.shape[2], labelData.shape[2], lobeLabelDim, hiddenDim, layerNum, maxSeqLen, splitPartNum, crossLenRate, maxLevelNum, cacheSize, maxLoopCount, batchSize, dropRate, trainData.shape[1], labelData.shape[1]).to(device)
    if os.path.exists(modulePath):
        model.LoadModuleWeight(modulePath)
        print(f"taskName = {taskName}, modulePath = {modulePath}, model.loss = {model.loss.item()}, model.varMinusRst = {model.varMinusRst}")
        if (model.loss.item() < 30) and (model.varMinusRst < 20):
            return model
...
if __name__ == "__main__":
    ...
    trainData = torch.randn(100, 300, 40)
    trainLabel = torch.randn(100, 20, 3)

    trainData = GenerateNormalizeRst(trainData)
    trainLabel = GenerateNormalizeRst(trainLabel)

    #TestSingleSfnNetWork(taskName, trainData, trainLabel)
    #TestSimpleLstmNetWork(taskName, trainData, trainLabel)
    model = TestSingleRrnnNetWork(taskName, trainData, trainLabel)
    #TestFuncSimulation(taskName)

    testData1 = torch.randn(100, 300, 40)
    testData2 = testData1.clone()
    ...
    '''
    for modifyIndexNum in range(1, 10, 2):
        for incr in range(0, 100, 10):
            for index in range(0, modifyIndexNum):
                rankIndex = int(random.random() * 10)
                colIndex = int(random.random() * 10)
                heightIndex = int(random.random() * 10)
                print(f"rankIndex = {rankIndex}, colIndex = {colIndex}, heightIndex = {heightIndex}")
                testData2[rankIndex][colIndex][heightIndex] = testData2[rankIndex][colIndex][heightIndex] + incr / 10.0
            result1, _, _ = model.GetModuleCalcRst(testData1, isNeedGrad=False)
            result2, _, _ = model.GetModuleCalcRst(testData2, isNeedGrad=False)
            print(f"incr = {incr}, modifyIndexNum = {modifyIndexNum}, oriNorm1 = {torch.norm(testData1, 2)}, oriNorm2 = {torch.norm(testData2, 2)}")
            print(f"incr = {incr}, modifyIndexNum = {modifyIndexNum}, norm1 = {torch.norm(result1, 2)}, norm2 = {torch.norm(result2, 2)}, diffNorm = {torch.norm(result1 - result2, 2)}")
    '''

    print(f"single ele change begin")
    testData2 = testData1.clone()
    for incr in range(0, 100, 2):
        testData2[1][2][3] = testData2[1][2][3] + incr / 10.0
        result1, _, _ = model.GetModuleCalcRst(testData1, isNeedGrad = False)
        result2, _, _ = model.GetModuleCalcRst(testData2, isNeedGrad = False)
        print(f"incr = {incr}, oriNorm1 = {torch.norm(testData1, 2)}, oriNorm2 = {torch.norm(testData2, 2)}")
        print(f"incr = {incr}, norm1 = {torch.norm(result1, 2)}, norm2 = {torch.norm(result2, 2)}, diffNorm = {torch.norm(result1 - result2, 2)}")
    print(f"single ele change end")

    print(f"single ele change begin")
    testData2 = testData1.clone()
    for incr in range(0, 100, 2):
        testData2[10][20][30] = testData2[10][20][30] + incr / 10.0
        result1, _, _ = model.GetModuleCalcRst(testData1, isNeedGrad = False)
        result2, _, _ = model.GetModuleCalcRst(testData2, isNeedGrad = False)
        print(f"incr = {incr}, oriNorm1 = {torch.norm(testData1, 2)}, oriNorm2 = {torch.norm(testData2, 2)}")
        print(f"incr = {incr}, norm1 = {torch.norm(result1, 2)}, norm2 = {torch.norm(result2, 2)}, diffNorm = {torch.norm(result1 - result2, 2)}")
    ...
    print(f"multi ele change begin")
    testData2 = testData1.clone()
    for incr in range(0, 100, 2):
        testData2[1][2][3] = testData2[1][2][3] + incr / 10.0
        testData2[50][20][30] = testData2[50][20][30] + incr / 10.0
        result1, _, _ = model.GetModuleCalcRst(testData1, isNeedGrad = False)
        result2, _, _ = model.GetModuleCalcRst(testData2, isNeedGrad = False)
        print(f"incr = {incr}, oriNorm1 = {torch.norm(testData1, 2)}, oriNorm2 = {torch.norm(testData2, 2)}")
        print(f"incr = {incr}, norm1 = {torch.norm(result1, 2)}, norm2 = {torch.norm(result2, 2)}, diffNorm = {torch.norm(result1 - result2, 2)}")
    print(f"multi ele change end")

    print(f"multi ele change begin")
    testData2 = testData1.clone()
    for incr in range(0, 100, 2):
        testData2[10][20][30] = testData2[10][20][30] + incr / 10.0
        testData2[80][20][30] = testData2[80][20][30] + incr / 10.0
        result1, _, _ = model.GetModuleCalcRst(testData1, isNeedGrad = False)
        result2, _, _ = model.GetModuleCalcRst(testData2, isNeedGrad = False)
        print(f"incr = {incr}, oriNorm1 = {torch.norm(testData1, 2)}, oriNorm2 = {torch.norm(testData2, 2)}")
        print(f"incr = {incr}, norm1 = {torch.norm(result1, 2)}, norm2 = {torch.norm(result2, 2)}, diffNorm = {torch.norm(result1 - result2, 2)}")
